...
type Theme = 'light' | 'dark' | 'tokyo-night' | 'tokyo-storm' | 'tokyo-light'
const savedTheme = localStorage.getItem('remotehub.theme')
const theme = ref<Theme>(savedTheme === 'light' || savedTheme === 'dark' || savedTheme === 'tokyo-night' || savedTheme === 'tokyo-storm' || savedTheme === 'tokyo-light' ? savedTheme : matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark')
...
<template>
  <div class="app-frame" :class="{ darwin: appInfo?.platform === 'darwin', fullscreen }">
    <header class="top-toolbar">
      ...
      <div class="toolbar-actions"><button class="toolbar-button" @click="openCreate">＋ {{ t('newConnection') }}</button><select v-model="theme" class="theme-select" :aria-label="t('themeLabel')" :title="t('themeLabel')" @change="applyTheme"><option value="dark">{{ t('darkMode') }}</option><option value="light">{{ t('lightMode') }}</option><option value="tokyo-night">Tokyo Night</option><option value="tokyo-storm">Tokyo Storm</option><option value="tokyo-light">Tokyo Light</option></select><button class="toolbar-button muted" @click="toggleLocale">{{ locale === 'zh-CN' ? 'EN' : '中文' }}</button></div>
    </header>
    <div class="app-body">
